...
async def rephrase_query(state: AgentState) -> dict[str, str]:
    ...
    try:
        rephrased = (
            await request.app.state.llm.complete(messages, temperature=0.0, max_tokens=120)
        ).strip()
    except Exception:
...
async def route(state: AgentState) -> dict[str, Any]:
    ...
    try:
        decision = (
            await request.app.state.llm.complete(messages, temperature=0.0, max_tokens=20)
        ).strip()
    except Exception:
