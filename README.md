...
- [ ] `http://localhost` → страница логина
- [ ] Логин admin работает
- [ ] Загрузка PDF → через ~30 сек статус "Индексирован"
- [ ] Поиск → результаты со сниппетом и `<mark>` подсветкой
- [ ] Превью PDF в Modal без скачивания
- [ ] Создать организацию → расшарить документ → другой пользователь находит
- [ ] AdminPage: все 4 вкладки с данными
